...
public class EdcWebServer {
    ...
    public static int run(String logger, int minRangePort, int maxRangePort) throws IOException {
        ...
        if (minRangePort < 0 || maxRangePort < 0)
            throw new IllegalArgumentException("the port range must be positive [" + minRangePort + ", " + maxRangePort + "]");
        if (minRangePort > maxRangePort)
            throw new IllegalArgumentException("the port range is badly defined [" + minRangePort + ", " + maxRangePort + "]");
